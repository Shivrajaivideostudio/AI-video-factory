...
# Initialize Owners
for email in OWNER_EMAILS: user_db[email] = {"status": "approved", "is_owner": True, "failed_otp": 0, "lockout": None}
for mob in OWNER_MOBILES: user_db[mob] = {"status": "approved", "is_owner": True, "failed_otp": 0, "lockout": None}
    ...
    async def process_command(command: str, user_context: dict):
        ...
        try:
            completion = openai_client.chat.completions.create(
                model="gpt-4o",
                messages=[{"role": "system", "content": "You are the AI Factory Lead Assistant."}, {"role": "user", "content": prompt}],
...
class VoiceEngine:
    ...
    async def generate_voice(text: str, output_path: str, voice: str = "en-US-AndrewNeural"):
        try:
            # Note: For Hindi use "hi-IN-MadhurNeural" or "hi-IN-SwararaNeural"
...
class FallbackEngine:
    ...
    def optimize_render(style: str):
        if style == "cinematic": return {"fps": 24, "bitrate": "15M", "filter": "color_grading_warm"}
...
async def send_otp(identifier: str = Form(...)):
    ...
    if identifier not in user_db:
        # In this system, only pre-approved owners or requested users can login
        # However, for simulation, we'll allow new users as 'pending'
...
async def verify_login(identifier: str = Form(...), otp: str = Form(...)):
    ...
    # OTP Verification
    if identifier not in otp_db or otp_db[identifier]["otp"] != otp or otp_db[identifier]["expiry"] < now:
        user["failed_otp"] += 1
...
async def create_video_page(request: Request, user: dict = Depends(get_current_user)):
    return templates.TemplateResponse("create_video.html", {"request": request, "user": user, "is_owner": user.get("is_owner", False)})
...
async def video_pipeline_task(user_id: str, topic: str, language: str, style: str, title: str):
    ...
    try:
        ...
        try:
            ...
            if fallback_data: ai_data.update(fallback_data)
        
        scenes = ai_data.get("scenes", [])
        if not scenes: raise Exception("Critical Scripting Failure")
        ...
        for idx, scene in enumerate(scenes):
            generation_status[user_id] = {"progress": 25 + int((idx / len(scenes)) * 40), "status": f"Scene {idx+1}/{len(scenes)}: AI Voice & Vision..."}
            ...
            if os.path.exists(audio_path) and os.path.exists(image_path):
                try:
                    ...
                    (
                        ...
                        .input(audio_path)
                        .output(scene_video, vcodec='libx264', acodec='aac', pix_fmt='yuv420p', 
                                r=24, preset='ultrafast', crf=28)
                        .overwrite_output()
                        .run(quiet=True)
                    )
        ...
        if not final_clips: raise Exception("Zero Scenes Rendered")
        ...
        with open(concat_file, "w") as f:
            for clip in final_clips: f.write(f"file '{os.path.abspath(clip)}'\n")
        ...
        zip_local = shutil.make_archive(os.path.join(job_path, f"package_{video_id}"), 'zip', job_path)
        ...
        if not video_link: raise Exception("Cloud Sync Failed")
...
async def update_user(identifier: str = Form(...), action: str = Form(...), user: dict = Depends(get_current_user)):
    if not user.get("is_owner"): raise HTTPException(status_code=403)
    
    if action == "approve": user_db[identifier]["status"] = "approved"
    elif action == "block": user_db[identifier]["status"] = "blocked"
    elif action == "unblock": user_db[identifier]["status"] = "approved"
    elif action == "blacklist": user_db[identifier]["status"] = "blacklisted"
    elif action == "delete": user_db.pop(identifier, None)
...
if __name__ == "__main__":
    import uvicorn
    # Railway के लिए पोर्ट डायनामिक होना चाहिए
